...
论文检索规则（Human-in-the-Loop）：
- 当用户的问题可能涉及论文内容时，**先**调用 \`suggest_paper_search\` 生成 3 个候选查询。
- 等待用户在交互卡片中选择查询和结果数量后，再调用 \`search_papers\` 执行实际检索。
- 如果用户选择「帮我选择」，直接调用 \`search_papers\` 使用你认为最佳的查询。
